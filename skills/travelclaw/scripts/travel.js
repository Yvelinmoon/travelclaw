...
function buildMatchContext(worldName, sceneName, injectContent, promptText) {
  const promptSnippets = String(promptText || '')
    .split(/[。.!?！？,，;；\n]/)
    .map((s) => s.trim())
    .filter(Boolean)
    .slice(0, 2);

  return [worldName, sceneName, injectContent, ...promptSnippets]
    .filter(Boolean)
    .join(' ');
}

function extractKeywords(worldName, sceneName, matchContext) {
  const combined = [worldName, sceneName, matchContext].filter(Boolean).join(' ');
  ...
  return unique(rawTokens.filter((w) => !stopwords.has(w))).slice(0, 8);
}

async function matchSkills(worldName, sceneName, matchContext) {
  const keywords = extractKeywords(worldName, sceneName, matchContext);
...
async function main() {
  ...
  const matchContext = buildMatchContext(worldName, sceneName, injectContent, promptText);
  const matchedSkills = await matchSkills(worldName, sceneName, matchContext);
  ...
  log(`🔍 Skill match world: ${worldName || '(empty)'}`);
  log(`🔍 Skill match keywords: ${extractKeywords(worldName, sceneName, matchContext).join(', ')}`);
  ...
  for (let i = 0; i < 360; i++) {
    ...
    if (result.task_status !== 'PENDING' && result.task_status !== 'MODERATION') {
      ...
      out({
        ...
        skill_match_query: {
          world: worldName,
          scene_keywords: extractKeywords(worldName, sceneName, matchContext),
        },
