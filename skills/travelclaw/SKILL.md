...
**触发场景：** awakening 流程的阶段 9 完成后自动触发。

**核心规则：**
- ✅ **跳过 Step 1-2**（角色信息已在 SOUL.md 中，世界观已在觉醒叙事中描述）
- ✅ **直接从 Step 3 开始**（Discord Opening：输出世界观揭幕 + 「去逛逛 🌀」按钮）
- ✅ **旅行进度从 1/5 开始计数**
- ✅ **无需用户额外对话**（觉醒叙事已完成沉浸感建立）

**执行流程：**
```
觉醒阶段 9 ⑥ → 旁白 + 世界观降临 + 角色问候 + 询问
     ↓
自动调用 travelclaw Step 3
     ↓
Step 3 → 输出完整世界观揭幕 + 「去逛逛 🌀」按钮
     ↓
用户点击 → Step 4 开始第 1 站旅行
```

**为什么可以跳过 Step 1-2？**
- SOUL.md 已在觉醒阶段更新（包含角色名 + 形象图片）
- 觉醒叙事中已描述世界观降临（无需再次搜索）
- 用户点击「去逛逛」即表示认可当前世界设定
