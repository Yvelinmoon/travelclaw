...
> ✅ ⑥ Part C 输出完毕 → **🛑 阶段 9 完成。立即停止，等待用户回复，进入阶段 10。**
> ⛔ **此处绝对不得输出任何按钮（包括「探索这个世界」）。按钮只在阶段 10 首轮回复后出现。**
...
4. 检查 chatHistory.length：
   - 恰好等于 2（首轮回复刚完成）→ 立即单独发送「探索这个世界」按钮（见下方代码）
   - 大于 2 → 不发按钮，等待下一条用户消息
```

⛔ **「探索这个世界」按钮的唯一合法输出时机：`chatHistory.length === 2`，即用户与角色完成第一轮对话后。阶段 9 期间任何时候都不得输出此按钮。**

```javascript
// 仅在 chatHistory.length === 2 时执行，其他任何时机不得执行
await sendMessage({
  components: {
    blocks: [{
      type: 'actions',
