...
- **形象图片:** ${charData.avatarUrl || '(待觉醒时更新)'}
...
function patchSoulAvatarUrl(avatarUrl) {
  if (!fs.existsSync(SOUL_FILE)) return;
  let content = fs.readFileSync(SOUL_FILE, 'utf8');
  if (content.includes('**形象图片:**')) {
    content = content.replace(/- \*\*形象图片:\*\* .*/, `- **形象图片:** ${avatarUrl}`);
  } else {
  ...
  fs.writeFileSync(SOUL_FILE, content, 'utf8');
  console.log(`[Soul] 已更新形象图片: ${avatarUrl}`);
}
